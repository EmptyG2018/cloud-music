...
export default {
    ...
    methods: {
        ...
        initPlayer () {
            const player = document.getElementsByClassName('__cov-video-container')[0]
            this.player.pos = player.getBoundingClientRect()
            this.player.oPlayer = player
        },
        initVol () {
            const oVolBox = document.getElementsByClassName('__cov-contrl-vol-slider')[0]
            const oVolInner = oVolBox.getElementsByClassName('__cov-contrl-vol-inner')[0]
            this.volume.oVolBox = oVolBox
            this.volume.pos.innerWidth = oVolInner.getBoundingClientRect().width
            this.volume.pos.start = oVolBox.getBoundingClientRect().left
            this.volume.pos.width = oVolBox.getBoundingClientRect().width - this.volume.pos.innerWidth
        },
        initVideo () {
            const oVideoSlider = document.getElementsByClassName('__cov-contrl-video-slider')[0]
            const oVideoInner = oVideoSlider.getElementsByClassName('__cov-contrl-video-inner')[0]
            this.oVideoSlider = oVideoSlider
            this.video.pos.start = oVideoSlider.getBoundingClientRect().left
            this.video.pos.innerWidth = oVideoInner.getBoundingClientRect().width
            this.video.pos.width = oVideoSlider.getBoundingClientRect().width - this.video.pos.innerWidth
            this.getTime()
        },
        mouseEnterVideo () {
        ...
        getTime () {
            this.oVideo.addEventListener('durationchange', (e) => {
                console.log(e)
            })
            this.oVideo.addEventListener('progress', (e) => {
                this.video.loaded = (-1 + (this.oVideo.buffered.end(0) / this.oVideo.duration)) * 100
            })
            this.video.len = this.oVideo.duration
        },
        setVideoByTime (percent) {
            this.oVideo.currentTime = Math.floor(percent * this.video.len)
        },
        play () {
            ...
            if (this.oVideo) {
                if (this.state.playing) {
                    ...
                    this.oVideo.addEventListener('timeupdate', this.timeline)
                    this.oVideo.addEventListener('ended', (e) => {
        ...
        timeline () {
            const percent = this.oVideo.currentTime / this.oVideo.duration
            this.video.pos.current = (this.video.pos.width * percent).toFixed(3)
            this.video.displayTime = timeParse(this.oVideo.duration - this.oVideo.currentTime)
        },
        volMove (e) {
            this.initVol()
        ...
        volMuted () {
            this.oVideo.muted = !this.oVideo.muted
            this.volume.muted = this.oVideo.muted
        },
        setVol (val) {
            if (this.oVideo) {
                this.volume.pos.current = val * this.volume.pos.width
                this.volume.percent = val * 100
                this.oVideo.volume = val
            }
        },
        fullScreen () {
            if (!this.state.fullScreen) {
                this.state.fullScreen = true
                this.oVideo.webkitRequestFullScreen()
            } else {
