	...
	<div class="home">
		...
		<div class="section selected-song-list">
			<div class="section-wrap">
				...
				<div class="list">
					<div class="list-over">
						<router-link class="item" v-for="item in PlayList.Recs" :key="item.id" :to="{name: 'songList', params: {id: item.id}}">
							<div class="thumb">
								<img :src="item.coverImgUrl" />
							</div>
							<div class="cell">
								<div class="desc">{{item.name}}</div>
								<div class="play-num"><mu-icon size="12" value="whatshot"></mu-icon> {{item.playCount}}</div>
							</div>
						</router-link>
					</div>
				</div>
...
export default {
	...
	methods: {
		async getTopPlayList() {
			...
			let topPlayList = await GetTopPlayList({ limit: PlayList.Limit, order: 'hot' });
