	...
	<div class='search'>
		<div class="search-top">
			...
			<div class="search">
				...
				<input class="search-controls" @keyup.enter="getSearchResult" @keyup="handleUpSongLen" v-model.trim="val" placeholder="这里有你爱听的歌，快搜索吧！" type="text" />
			</div>
		</div>
		<div class="section songs-result">
			<div class="songs-len" v-show="songLenShow">关于"<span style='color: #F14F48'>{{val}}</span>"的歌曲，共{{data.count}}首歌曲</div>
			<div class="">
...
export default {
	...
	methods: {
		async getSearchResult() {
			...
			[this.data.count, this.data.list] = [searchResult.result.songCount, searchResult.result.songs];
